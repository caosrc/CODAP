...
export async function exportarFiscalizacaoExcel(registrosCurral: CurralRegistro[], relatoriosProcon: ProconRegistro[]) {
  ...
  resumo.addRows([
    ...
    ['Registros Curral', registrosCurral.length],
    ['Registros Procon', relatoriosProcon.length],
    ['Total de ocorrências', registrosCurral.length + relatoriosProcon.length],
  ])
  ...
  criarPlanilha('Curral', 'OCORRÊNCIAS DO CURRAL — UMA LINHA POR REGISTRO', curralHeaders, curralRows(registrosCurral))
  criarPlanilha('Procon', 'AÇÕES DO PROCON — UMA LINHA POR DOCUMENTO', proconHeaders, proconRows(relatoriosProcon))

  const buffer = await workbook.xlsx.writeBuffer()
  baixarArquivo(buffer, `fiscalizacao_codap_${new Date().toISOString().slice(0, 10)}.xlsx`, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
  return { total: registrosCurral.length + relatoriosProcon.length }
}
...
export async function exportarFiscalizacaoDocx(registrosCurral: CurralRegistro[], relatoriosProcon: ProconRegistro[]) {
  const JSZip = (await import('jszip')).default
  const zip = new JSZip()
  const todasLinhas = [
    ...curralRows(registrosCurral),
    ...proconRows(relatoriosProcon),
  ]
  const cabecalho = ['Módulo', 'ID', 'Status', 'Documento / espécie', 'Número / processo', 'Estabelecimento / local', 'Data', 'Latitude', 'Longitude']
  const linhas = [
    ...registrosCurral.map((registro) => ['Curral', registro.id, registro.status, registro.especie, '', registro.localDescricao, dataBr(registro.capturadoEm), registro.latitude, registro.longitude]),
    ...relatoriosProcon.map((registro) => ['Procon', registro.id, statusProcon(registro.status), nomeDocumento(registro.identificacao.tipoDocumento), `${registro.identificacao.numero}${registro.identificacao.numeroProcesso ? ` / ${registro.identificacao.numeroProcesso}` : ''}`, registro.estabelecimento.nomeFantasia || registro.estabelecimento.razaoSocial, dataBr(registro.identificacao.horaInicio), registro.localizacao.latitude, registro.localizacao.longitude]),
  ]
  const documentXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${paragrafo('RELATÓRIO DE FISCALIZAÇÃO — CODAP', 'Title')}${paragrafo(`Gerado em ${dataBr(new Date().toISOString())}. ${todasLinhas.length} ocorrência(s), sendo ${registrosCurral.length} do Curral e ${relatoriosProcon.length} do Procon.`)}${paragrafo('Registros linha a linha', 'Heading1')}${tabela(cabecalho, linhas)}${paragrafo('Detalhamento do Curral', 'Heading1')}${tabela(['ID', 'Espécie', 'Porte', 'Sexo', 'Identificação', 'Local', 'Data', 'GPS', 'Observações'], registrosCurral.map((registro) => [registro.id, registro.especie, registro.porte, registro.sexo, registro.identificacao, registro.localDescricao, dataBr(registro.capturadoEm), `${texto(registro.latitude)}, ${texto(registro.longitude)}`, registro.observacoes]))}${paragrafo('Detalhamento do Procon', 'Heading1')}${tabela(['ID', 'Documento', 'Número', 'Processo', 'Estabelecimento', 'Município', 'Data', 'Irregularidades', 'Resultado'], relatoriosProcon.map((registro) => [registro.id, nomeDocumento(registro.identificacao.tipoDocumento), registro.identificacao.numero, registro.identificacao.numeroProcesso, registro.estabelecimento.nomeFantasia || registro.estabelecimento.razaoSocial, registro.endereco.municipio, dataBr(registro.identificacao.horaInicio), registro.irregularidades.length, registro.resultado.itens.join('; ')]))}<w:sectPr><w:pgSz w:w="16838" w:h="11906" w:orient="landscape"/><w:pgMar w:top="720" w:right="720" w:bottom="720" w:left="720"/></w:sectPr></w:body></w:document>`
  ...
  baixarArquivo(buffer, `fiscalizacao_codap_${new Date().toISOString().slice(0, 10)}.docx`, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')
  return { total: todasLinhas.length }
}
...
export async function exportarFiscalizacaoKmz(registrosCurral: CurralRegistro[], relatoriosProcon: ProconRegistro[]): Promise<ExportResult> {
  const JSZip = (await import('jszip')).default
  let pontos = 0
  let semGps = 0
  const curralPlacemarks = registrosCurral.map((registro) => {
  ...
  const proconPlacemarks = relatoriosProcon.map((registro) => {
  ...
  const kml = `<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>Fiscalização CODAP</name><description>Locais de animais apreendidos e ações do Procon. Clique em cada ponto no Google Earth para consultar as informações básicas.</description><Folder><name>Curral — animais apreendidos</name>${curralPlacemarks}</Folder><Folder><name>Procon — ações de fiscalização</name>${proconPlacemarks}</Folder></Document></kml>`
  const zip = new JSZip()
  zip.file('fiscalizacao.kml', kml)
  const buffer = await zip.generateAsync({ type: 'blob', mimeType: 'application/vnd.google-earth.kmz' })
  baixarArquivo(buffer, `fiscalizacao_codap_${new Date().toISOString().slice(0, 10)}.kmz`, 'application/vnd.google-earth.kmz')
  return { total: registrosCurral.length + relatoriosProcon.length, pontos, semGps }
}